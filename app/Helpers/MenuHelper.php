...
class MenuHelper
{
    public static function getMainNavItems()
    {
        return [
            ...
            [
                ...
                'icon' => 'user-profile',
                'subItems' => [
                    ['name' => 'Kelola User', 'path' => '/users', 'pro' => false, 'permission' => 'users'],
                    ['name' => 'Kelola Role', 'path' => '/roles', 'pro' => false, 'permission' => 'roles'],
                    ['name' => 'Pengaturan Tampilan', 'path' => '/pengaturan/tampilan', 'pro' => false, 'permission' => 'app-settings'],
                    ['name' => 'Pengaturan WhatsApp', 'path' => '/pengaturan/whatsapp', 'pro' => false, 'permission' => 'whatsapp-settings'],
                    ['name' => 'Pengaturan Google Drive', 'path' => '/pengaturan/google-drive', 'pro' => false, 'permission' => 'google-drive-settings'],
                ],
            ],
            [
                'icon' => 'user-profile',
    ...
    public static function permissionGroups()
    {
        return [
            ...
            'Administrasi' => [
                'users' => 'Kelola User',
                'roles' => 'Kelola Role',
                'app-settings' => 'Pengaturan Tampilan',
                'whatsapp-settings' => 'Pengaturan WhatsApp',
                'google-drive-settings' => 'Pengaturan Google Drive',
            ],
        ];
    }
    ...
    {
        return collect(self::permissionGroups())->collapse()->all();
    }

    protected static function canSeeItem($item)
    {
        return empty($item['permission']) || (auth()->check() && auth()->user()->can($item['permission']));
    }

    public static function getMenuGroups()
    {
        $items = collect(self::getMainNavItems())
            ->filter(fn ($item) => self::canSeeItem($item))
            ->map(function ($item) {
                if (isset($item['subItems'])) {
                    $item['subItems'] = collect($item['subItems'])
                        ->filter(fn ($subItem) => self::canSeeItem($subItem))
                        ->values()
                        ->all();
                }
            ...
            ->filter(fn ($item) => ! isset($item['subItems']) || count($item['subItems']) > 0)
            ->values()
            ->all();
