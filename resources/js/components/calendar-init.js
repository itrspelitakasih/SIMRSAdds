...
export function calendarInit() {
  ...
  if (calendarWrapper) {
    ...
    // Reminder Dokumen events, injected server-side via the #calendar
    // element's data-document-events attribute (see calender-area.blade.php)
    let calendarEventsList = [];
    try {
      calendarEventsList = JSON.parse(calendarEl.dataset.documentEvents || "[]");
    } catch (e) {
      calendarEventsList = [];
    }
