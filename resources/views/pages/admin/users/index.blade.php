            ...
            <div class="max-w-full overflow-x-auto custom-scrollbar">
                <table class="w-full min-w-[500px]">
                    ...
                    <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                        @forelse ($users as $user)
                            <tr>
                                <td class="px-5 py-4 sm:px-6">
                                    <p class="text-gray-700 text-theme-sm dark:text-gray-300">{{ $user->name }}</p>
                                </td>
                                <td class="px-5 py-4 sm:px-6">
                                ...
                                <td class="px-5 py-4 sm:px-6">
                                    @forelse ($user->roles as $role)
                                        <x-ui.badge :color="$role->name === 'Admin' ? 'primary' : 'light'" size="sm">{{ $role->name }}</x-ui.badge>
                                    @empty
                                ...
                                <td class="px-5 py-4 sm:px-6">
                                    <div class="flex justify-end relative">
                                        <x-common.table-dropdown>
                                            ...
                                            <x-slot name="content">
                                                <a href="{{ route('users.edit', $user) }}" class="flex w-full px-3 py-2 font-medium text-left text-gray-500 rounded-lg text-theme-xs hover:bg-gray-100 hover:text-gray-700 dark:text-gray-400 dark:hover:bg-white/5 dark:hover:text-gray-300">Ubah</a>
                                                @if ($user->id !== auth()->id())
                        ...
                        @empty
                            <tr><td colspan="4" class="px-5 py-6 text-center text-gray-400">Belum ada user.</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
